...
def UserFactorUpdateWorker(out_q, lo, hi, beta, theta_p, theta_n,
           ...
           M, XP, FXP, XN, FXN, BTBpR, c0, c1, f, mu_u_p, mu_u_n, mode):
    ...
    if mode == None:
        ...
        for ui, u in enumerate(xrange(lo, hi)):
            ...
            y_u, idx_y_u = get_row(XP, u)
            T_j = theta_p[idx_y_u]

            rsd = y_u - bias_b_p[u] - bias_c_p[idx_y_u] - global_y_p

            if FXP is not None:  # FY is weighted matrix of Y
                f_u, _ = get_row(FXP, u)
            ...
            a = m_u.dot(c1 * B_p) + \
                np.dot(rsd, T_j) #it should be mu_u_p * np.dot(rsd, T_j) but we put more weight for this
        ...
        for ui, u in enumerate(xrange(lo, hi)):
            ...
            y_u, idx_y_u = get_row(XN, u)
            T_j = theta_n[idx_y_u]

            rsd = y_u - bias_b_n[u] - bias_c_n[idx_y_u] - global_y_n

            if FXN is not None:  # FY is weighted matrix of Y
                f_u, _ = get_row(FXN, u)
                TTT = T_j.T.dot(T_j * f_u[:, np.newaxis])
                rsd *= f_u
            else:
        ...
        for ui, u in enumerate(xrange(lo, hi)):
            ...
            a = m_u.dot(c1 * B_p) + mu_u_p * np.dot(rsd_p, T_j_p) + \
                                                mu_u_n * np.dot(rsd_n, T_j_n)
            A = BTBpR + B_p.T.dot((c1 - c0) * B_p) + TTT_p + TTT_n
            alpha_batch[ui] = LA.solve(A, a)
        ...
        for i in range(0, self.n_jobs):
            ...
            p = multiprocessing.Process(
                ...
                args=(out_q, lo, hi, self.beta, self.theta_p, self.theta_n,
                      ...
                      self.M, self.XP, self.FXP, self.XN, self.FXN,
...
def ProjectFactorUpdateWorker(out_q, lo, hi,
    ...
    if mode == None:
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            x_pj, idx_x_j = get_row(XP, p)
            G_i = gamma_p[idx_x_j]

            rsd = x_pj - bias_d_p[p] - bias_e_p[idx_x_j] - global_x_p
            ...
            a = m_u.dot(c1 * A_u) + mu_p_p * np.dot(rsd, G_i)
            beta_batch[pi] = LA.solve(B, a)
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            x_pj, idx_x_j = get_row(XN, p)
            G_i = gamma_n[idx_x_j]

            rsd = x_pj - bias_d_n[p] - bias_e_n[idx_x_j] - global_x_n
            ...
            B = TTTpR + A_u.T.dot((c1 - c0) * A_u) + GTG
            a = m_u.dot(c1 * A_u) + mu_p_n * np.dot(rsd, G_i)
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            B = TTTpR + A_u.T.dot((c1 - c0) * A_u) + \
                GTG_p + GTG_n
                # mu_p_p *GTG_p + mu_p_n *GTG_n
            a = m_u.dot(c1 * A_u) + mu_p_p * np.dot(rsd_p, G_i_p) + \
                                    mu_p_n * np.dot(rsd_n, G_i_n)
