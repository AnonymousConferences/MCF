...
PROJECT_INFO_PATH = "data/project-info/project-fundingduration.csv"
...
project_map.to_csv('data/rec_data/project_to_id.csv', index=False)
...
OUTPUT_RESTRUCTURED_USER_BACKEDPROJECT_FILE = 'data/rec_data/user_backed_projects_ts.csv'
...
if BOOLEAN_RESTRUCTURE_USER_BACKED_FILE:
    ...
    for index, row in project_info_data.iterrows():
        project_id_map[row['project-id']] = (row['pid'], row['timestamp'], row['timestamp_end'])


    fr = open(USER_BACKED_INFO_FILE, 'r')
    fw = open(OUTPUT_RESTRUCTURED_USER_BACKEDPROJECT_FILE, 'w')
    fw.write('bid,pid,timestamp,timestamp_end\n')
    backer_set = set()
    backer_dic = dict()
    bid = -1
    for line in fr:
        ...
        for backed_project in tokens[1:]:
            pid, timestamp, timestamp_end = project_id_map.setdefault(backed_project, ('','',''))
            new_line = str(local_bid) + ',' + str(pid) + ',' + str(timestamp) + ',' + str(timestamp_end) +'\n'
            fw.write(new_line)
    fr.close()
    fw.flush()
    fw.close()
...
pid_ts_tmp = user_pro_data[['pid','timestamp','timestamp_end']]
if REVERSE:
...
print user_pro_data
user_pro_data = user_pro_data[['new_bid', 'new_pid', 'timestamp','timestamp_end']]
user_pro_data.columns = ['bid', 'pid', 'timestamp', 'timestamp_end']
print user_pro_data
...
with open(os.path.join(DATA_DIR, CONTAINED_DIR, 'unique_uid.txt'), 'w') as f:
    for uid in unique_uid:
        f.write('%s\n' % uid)
with open(os.path.join(DATA_DIR, CONTAINED_DIR, 'unique_pid.txt'), 'w') as f:
