...
if TUNING:
    ...
    if model_type == 'mcf':
        tuner.run_alone("mcf", n_jobs = n_jobs, max_iter = 50, mu_p_p = 1.0, mu_p_n = 1.0, vad_K=100)
    ...
    if model_type == 'separatedmcf':
        tuner.run_alone("separatedmcf", n_jobs = n_jobs, max_iter = 30, mu_u_p = 1.0,  mu_p_p = 1.0, mu_p_n = 1.0, vad_K=100)
end = time.time()
