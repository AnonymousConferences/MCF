...
class ModelTuning:
    ...
    def local_alone_eval(self, U, V):
        ...
        for K in [5, 10, 20, 50, 100]:
            recall_at_K = rec_eval.parallel_recall_at_k(self.train_data, self.test_data, U, V, k=K,
                                                        vad_data=self.vad_data, n_jobs=16, clear_invalid=True)
    ...
    def local_eval(self, U, V, best_ndcg_10):
        ...
        is_better = False
        for K in [5, 10, 20, 50, 100]:
            recall_at_K = rec_eval.parallel_recall_at_k(self.train_data, self.test_data, U, V, k=K, vad_data=self.vad_data, n_jobs=16)
            print 'Test Recall@%d: %.4f' % (K, recall_at_K)
            ndcg_at_K = rec_eval.parallel_normalized_dcg_at_k(self.train_data, self.test_data, U, V, k=K, vad_data=self.vad_data, n_jobs=16)
            print 'Test NDCG@%d: %.4f' % (K, ndcg_at_K)
            map_at_K = rec_eval.parallel_map_at_k(self.train_data, self.test_data, U, V, k=K, vad_data=self.vad_data, n_jobs=16)
            print 'Test MAP@%d: %.4f' % (K, map_at_K)
            if K == 10:
    ...
    def run_alone(self, type, n_jobs = 8, n_components = 100, max_iter = 50, vad_K = 100, **kwargs):
        ...
        if type == 'cofactor':
            ...
            coder = cofactor.CoFacto(
                ...
                c1=c1)
            coder.fit(self.train_data, self.X_sppmi, vad_data=self.vad_data, batch_users=5000, k=vad_K,
            ...
            model_out_name = 'Cofactor_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
        ...
        if type == 'model2':
            ...
            coder = mymodel2.ParallelMFPosUserPosProjectEmbedding(mu_u = mu_u, mu_p = mu_p,
                             ...
                             random_state=98765, save_params=True, save_dir=self.save_dir, early_stopping=True, verbose=True,
                             lambda_alpha = lam_alpha, lambda_theta=lam_theta, lambda_beta=lam_beta, lambda_gamma=lam_gamma, c0=c0, c1=c1)
            coder.fit(self.train_data, self.X_sppmi, self.Y_sppmi, vad_data=self.vad_data, batch_users=5000, k=vad_K,
                      clear_invalid=True, n_jobs=16)
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Model2_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz'%(recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        ...
        if type == 'model3':
            ...
            coder.fit(self.train_data, self.X_sppmi, self.X_neg_sppmi, vad_data=self.vad_data, batch_users=5000, k=vad_K,
                      clear_invalid=True, n_jobs=16)
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Model3_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        ...
        if type == 'model4':
            ...
            coder.fit(self.train_data, self.Y_sppmi, self.Y_neg_sppmi, vad_data=self.vad_data, batch_users=5000, k=vad_K,
                      clear_invalid=True, n_jobs=16)
        ...
        if type == 'mcf':
            ...
            coder.fit(self.train_data, self.X_sppmi, self.X_neg_sppmi, self.Y_sppmi,
                      vad_data=self.vad_data, batch_users=5000, k=vad_K, clear_invalid=True, n_jobs=16)
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Model5_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        if type == 'model6':
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Model6_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        if type == 'model7':
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Model7_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        if type == 'separatedmodel2':
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Separated_Model2_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
        if type == 'separatedmodel3':
            ...
            coder.fit(self.train_data, self.X_sppmi, self.X_neg_sppmi, vad_data=self.vad_data, batch_users=5000,
                      k=vad_K,
                      clear_invalid=True, n_jobs=16)
        ...
        if type == 'separatedmcf':
            ...
            U, V = params['U'], params['V']
            (recall100, ndcg100, map100) = self.local_alone_eval(U, V)
            model_out_name = 'Separated_Model5_K100_recall100_%.4f_ndcg100_%.4f_map100_%.4f.npz' % (recall100, ndcg100, map100)
            np.savez(model_out_name, U=U, V=V)
