...
class CoFacto(BaseEstimator, TransformerMixin):
    def __init__(self, mu=1.0, n_components=100, max_iter=10, batch_size=1000,
    ...
    def _update_factors(self, X, XT, M, F):
        ...
        self.beta = update_beta(self.theta, self.gamma,
                                ...
                                self.n_jobs,
                                batch_size=self.batch_size, mu = self.mu)
        if self.verbose:
    ...
    def _update_biases(self, M, F):
        ...
        self.bias_b = update_bias(self.beta, self.gamma,
                                  ...
                                  self.n_jobs, batch_size=self.batch_size, mu = self.mu)
        # here it really should be M^T and F^T, but both are symmetric
...
def update_beta(theta, gamma, bias_b, bias_g, alpha, XT, M, F, c0, c1,
                lam_beta, n_jobs, batch_size=1000, mu = 1.0):
...
def _solve_weighted_cofactor(lo, hi, theta, gamma, bias_b, bias_g, alpha, XT,
    ...
    for ib, i in enumerate(xrange(lo, hi)):
        ...
        B = TTTpR + T_i.T.dot((c1 - c0) * T_i) + mu*GTG
        a = x_i.dot(c1 * T_i) + mu*np.dot(rsd, G_i)
...
def update_gamma(beta, bias_b, bias_g, alpha, MT, FT, lam_gamma,
    ...
    res = Parallel(n_jobs=n_jobs)(
        delayed(_solve_factor)(
            lo, hi, beta, bias_b, bias_g, alpha, MT, FT, f, lam_gamma, mu)
        for lo, hi in zip(start_idx, end_idx))
    gamma = np.vstack(res)
    return gamma
...
def update_bias(beta, gamma, bias_g, alpha, M, F, n_jobs, batch_size=1000, mu=1.0):
    ...
    res = Parallel(n_jobs=n_jobs)(
        delayed(_solve_bias)(lo, hi, beta, gamma, bias_g, alpha, M, F, mu)
        for lo, hi in zip(start_idx, end_idx))
    bias_b = np.hstack(res)
    return bias_b


def _solve_bias(lo, hi, beta, gamma, bias_g, alpha, M, F, mu = 1.0):
    ...
    for ib, i in enumerate(xrange(lo, hi)):
        ...
        if rsd.size > 0:
            bias_b_batch[ib] = mu*rsd.mean()
        else:
            bias_b_batch[ib] = 0.
    return bias_b_batch


def update_alpha(beta, gamma, bias_b, bias_g, M, F, n_jobs, batch_size=1000, mu = 1.0):
    ...
    res = Parallel(n_jobs=n_jobs)(
        delayed(_solve_alpha)(lo, hi, beta, gamma, bias_b, bias_g, M, F, mu)
        for lo, hi in zip(start_idx, end_idx))

    return np.sum(res) / M.data.size


def _solve_alpha(lo, hi, beta, gamma, bias_b, bias_g, M, F, mu = 1.0):
    res = 0.
    for ib, i in enumerate(xrange(lo, hi)):
        m_i, idx_i = get_row(M, i)
