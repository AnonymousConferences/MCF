...
class SeparatedParallelMFPosUserPosProjectEmbedding(BaseEstimator, TransformerMixin):
    def __init__(self, mu_u_p = 1.0, mu_p_p = 1.0, n_components=100, max_iter=10, batch_size=1000,
    ...
    def fit(self, M, XP = None, YP = None,
            FXP=None, FYP = None, vad_data=None, **kwargs):
        ...
        self._update(M, XP, YP, FXP, FYP, vad_data, **kwargs)
        return self

    def transform(self, M):
        pass

    def _update(self, M, XP, YP,  FXP, FYP, vad_data, **kwargs):
        ...
        for i in xrange(self.max_iter):
            if self.verbose:
                print('ITERATION #%d' % i)
            self._update_separate_item_factors(XP, XPT, FXP, FXPT)
            self._update_item_biases(XP, XPT, FXP, FXPT)
    ...
    def _update_separate_item_factors(self, XP, XPT, FXP, FXPT):
        if self.verbose:
            start_t = _writeline_and_time('\tUpdating project factors...')
        self.beta = update_beta(None, self.gamma_p,
                                self.bias_d_p, self.bias_e_p, self.global_x_p,
                                None, XP, FXP, self.c0, self.c1, self.lam_beta,
                                self.n_jobs, batch_size=self.batch_size,
                                mu_p_p = self.mu_p_p, f = self.n_components)
        ...
        if self.verbose:
            print('\r\tUpdating positive project embedding factors: time=%.2f'
                  % (time.time() - start_t))
    def _update_separate_user_factors(self, M, YP, YPT, FYP, FYPT):
        if self.verbose:
        ...
        self.theta_p = update_embedding_factor(self.alpha,
                                             ...
                                             mu_p=self.mu_u_p)
        if self.verbose:
            print('\r\tUpdating positive user embedding factors: time=%.2f'
                  % (time.time() - start_t))


    def _update_factors(self, M, MT, XP, XPT, YP, YPT,
                        FXP, FXPT, FYP, FYPT):
        if self.verbose:
            start_t = _writeline_and_time('\tUpdating user factors...')
        ...
        if self.verbose:
            print('\r\tUpdating user factors: time=%.2f'
                  % (time.time() - start_t))
            start_t = _writeline_and_time('\tUpdating project factors...')
        self.beta = update_beta(self.alpha, self.gamma_p,
                                self.bias_d_p, self.bias_e_p, self.global_x_p,
                                MT, XP, FXP, self.c0, self.c1, self.lam_beta,
                                self.n_jobs, batch_size=self.batch_size,
                                mu_p_p = self.mu_p_p)
        # print('checking project factor isnan : %d' % (np.sum(np.isnan(self.beta))))
        if self.verbose:
    ...
    def _update_item_biases(self, XP, XPT, FXP, FXPT):
        if self.verbose:
            start_t = _writeline_and_time('\tUpdating item bias terms...')
        ...
        self.global_x_p = update_global(self.beta, self.gamma_p,
                                  ...
                                  mu=self.mu_p_p)
        if self.verbose:
            print('\r\tUpdating item bias terms: time=%.2f'
                  % (time.time() - start_t))
        pass
    def _update_user_biases(self, YP, YPT, FYP, FYPT):
        if self.verbose:
            start_t = _writeline_and_time('\tUpdating user bias terms...')
        ...
        if self.verbose:
            print('\r\tUpdating user bias terms: time=%.2f'
                  % (time.time() - start_t))
        pass
    ...
    def _save_params(self):
        ...
        np.savez(os.path.join(self.save_dir, filename), U=self.alpha, V=self.beta)
