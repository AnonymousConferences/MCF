...
def load_data(csv_file, shape=(n_users, n_projects)):
    ...
    #take only user and song with count >= 2
    tp =  tp[(tp['rating'] <= 20) | tp['rating'] == 255]
    count, rows, cols = np.array(tp['rating']), np.array(tp['uid']), np.array(tp['sid']) #rows will be user ids, cols will be projects-ids.
...
for  i in range(1,5):
    ...
    if GENERATE_PROJECT_PROJECT_COOCCURENCE_FILE:
        t1 = time.time()
        print 'Generating project project negative_co-occurrence matrix'
    ...
    if GENERATE_USER_USER_COOCCURENCE_FILE:
        t1 = time.time()
        print 'Generating user user negative_ co-occurrence matrix'
    ...
    if BOOLEAN_LOAD_UU_COOCC_FROM_FILE:
        ...
        print 'dumping matrix ...'
        t1 = time.time()
        text_utils.save_pickle(Y, os.path.join(DATA_DIR, 'negative_user_user_cooc_fold%d.dat'%FOLD))
        t2 = time.time()
        print 'Time : %d seconds'%(t2-t1)
    else:
        print 'test loading model from pickle file'
        t1 = time.time()
        Y = text_utils.load_pickle(os.path.join(DATA_DIR, 'negative_user_user_cooc_fold%d.dat'%FOLD))
        t2 = time.time()
        print '[INFO]: sparse matrix size of user user negative_co-occurrence matrix: %d mb\n' % (
