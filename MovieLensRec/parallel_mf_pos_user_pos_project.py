...
class ParallelMFPosUserPosProjectEmbedding(BaseEstimator, TransformerMixin):
    ...
    def _update(self, M, XP, YP,  FXP, FYP, vad_data, **kwargs):
        ...
        for i in xrange(self.max_iter):
            if self.verbose:
                print('ITERATION #%d' % i)
            self._update_factors(M, MT, XP, XPT, YP, YPT,
                                 FXP, FXPT, FYP, FYPT)
            self._update_biases(XP, XPT, YP, YPT,
                                FXP, FXPT, FYP, FYPT)
    ...
    def _update_biases(self, XP, XPT, YP, YPT,
                       FXP, FXPT, FYP, FYPT):
        if self.verbose:
...
def update_beta(alpha, gamma_p,
                ...
                MT, XP, FXP,
                c0, c1, lam_beta,
                n_jobs, batch_size=1000, mu_p_p = 1.0):
