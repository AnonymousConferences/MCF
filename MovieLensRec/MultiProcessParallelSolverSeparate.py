...
def UserFactorUpdateWorker(out_q, lo, hi, beta, theta_p, theta_n,
    ...
    if mode == None:
        ...
        for ui, u in enumerate(xrange(lo, hi)):
            ...
            a = m_u.dot(c1 * B_p) + c1*mu_u_p * np.dot(rsd_p, T_j_p) + \
...
class UpdateProjectFactorParallel:
    def __init__(self,
                 alpha = None, gamma_p = None, gamma_n = None,
        ...
        self.mode = mode            # value for this param is : None, positive, negative, hybrid
                                    ...
                                    # negative means applying negative embedding for this para_type
                                    # hybrid: combine positive + negative embeddding
        self.n = gamma_p.shape[0]  # n: number of projects, m: number of users



    def run(self):
        out_q = Queue()
        beta = np.zeros((self.n, self.f), dtype=self.gamma_p.dtype)
...
def ProjectFactorUpdateWorker(out_q, lo, hi,
                              ...
                              mu_p_p, mu_p_n, mode):
    beta_batch = np.zeros((hi - lo, f), dtype=gamma_p.dtype)
    if mode == None:
        print 'error while updating project latent factors'
        sys.exit(1)
    ...
    if mode == 'positive':
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            if FXP is not None:
                ...
                GTG = G_i.T.dot(G_i)
            GTG = c1*mu_p_p*GTG #add new
            B = TTTpR + GTG
            a = c1 * mu_p_p * np.dot(rsd, G_i)
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            if FXN is not None:
                ...
                GTG = G_i.T.dot(G_i)
            GTG = c1*mu_p_n*GTG #add new
            B = TTTpR + GTG
            a = c1 * mu_p_n * np.dot(rsd, G_i) #modify
        ...
        for pi, p in enumerate(xrange(lo, hi)):
            ...
            a = c1*mu_p_p * np.dot(rsd_p, G_i_p) + \
