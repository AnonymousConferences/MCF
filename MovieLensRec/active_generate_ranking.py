...
def load_data(csv_file, shape=(n_users, n_projects)):
    ...
    timestamps, rows, cols = np.array(tp['timestamp']), \
                                             np.array(tp['userId']), np.array(tp['movieId']) #rows will be user ids, cols will be projects-ids.
...
#print user_activity
#idx_active = [i for i,count in enumerate(user_activity) if count > 15]
#print idx_active
...
if len(glob.glob(os.path.join(DATA_DIR, 'project_ts_df.csv'))) <= 0:
    project_ts_df_train = train_df[['movieId', 'timestamp']]
    # project_ts_df_train = project_ts_df_train.drop_duplicates(cols='pid').sort_index(by='pid', ascending=True)
    project_ts_df_train = project_ts_df_train.drop_duplicates('movieId').sort_index(by='movieId', ascending=True)
    project_ts_df_vad = vad_df[['movieId', 'timestamp']]
    # project_ts_df_vad = project_ts_df_vad.drop_duplicates(cols='pid').sort_index(by='pid', ascending=True)
    project_ts_df_vad = project_ts_df_vad.drop_duplicates('movieId').sort_index(by='movieId', ascending=True)
    project_ts_df_test = test_df[['movieId', 'timestamp']]
    # project_ts_df_test = project_ts_df_test.drop_duplicates(cols='pid').sort_index(by='pid', ascending=True)
    project_ts_df_test = project_ts_df_test.drop_duplicates('movieId').sort_index(by='movieId', ascending=True)
    test_data.data = np.ones_like(test_data.data)
    project_ts_df = pd.concat([project_ts_df_train, project_ts_df_test, project_ts_df_vad])
    # project_ts_df = project_ts_df.drop_duplicates(cols='pid').sort_index(by='pid', ascending=True)
    project_ts_df = project_ts_df.drop_duplicates('movieId').sort_index(by='movieId', ascending=True)
    project_ts_df_train = None
    project_ts_df_vad = None
    project_ts_df_test = None
    project_ts_df = project_ts_df.reset_index(drop=True)
...
#active_condition = [200]
#active_condition = [0,25,50]
for model_name in model_names:
    t1 = time.time()
    for i in range(10):
        if 'fold%d'%i in model_name:
            train_data, train_raw, train_df = load_data(os.path.join(DATA_DIR, 'train_fold%d.csv'%i))
            break
    user_activity = np.asarray(train_data.sum(axis=1)).ravel()
    ...
    for i in range(len(active_condition)):
        ...
        if i == (len(active_condition) - 1): idx_active = [j for j, count in enumerate(user_activity) if count >= active_from]
        else:
        ...
        train_data = train_data[idx_active, :]
        test_data = test_data[idx_active, :]
        vad_data = vad_data[idx_active, :]
        #if active_to > active_from:
        #    print 'Original ranking with condition >= %d and < %d'%(active_from, active_to)
        #else: print 'Original ranking with condition >= %d'%(active_from)
        for K in topk_range:
